...
acAnimated.Wall = function(wall) {
  ...
  var slidesNum = wall.querySelectorAll('.slide').length
  ...
  for (var i=0; i<=slides.length-1; i++) {
    ...
    timeline
    .add("slide_" + String(i + 1), acAnimated.randomNumber(0, slidesNum) / 10)
    ...
    slide.onclick = function() {
	  ...
	  .to(this.querySelector(".image1"), 0.1, {backgroundSize: "auto"})
      .to(this, 0.5, {left: "0%", top: "0%", width: "100%", height: "100%", zIndex: "2"}, "play_timeline");
      ...
      closeButton.onclick = function() {
        ...
        for (var i=0; i<=slides.length-1; i++) {
          ...
          timeline
          .to(_slide, 0.5, {top: String((100 / rows) * _slide.row) + "%", left: String((100 / columns) * _slide.column) + "%", width: String(100 / columns) + "%", height: String(100 / rows) + "%", zIndex: "0"}, "play_timeline");
        }
        
        timeline
		.to(this.querySelector(".image1"), 0.1, {backgroundSize: "cover"})
